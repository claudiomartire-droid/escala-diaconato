...
arquivos_historicos = st.sidebar.file_uploader("Suba o historico_consolidado.csv ou escalas anteriores", type=["csv", "xlsx"], accept_multiple_files=True)
...
if arquivo_carregado:
    ...
    # --- LÓGICA DE EQUIDADE CONTÍNUA ---
    # Inicializa o histórico com 0 ou com os dados dos arquivos carregados
    ...
    if arquivos_historicos:
        for arq in arquivos_historicos:
            try:
                ...
                # Se for o historico_consolidado.csv gerado pelo próprio app:
                if 'historico_ceia' in df_h.columns and 'Nome' in df_h.columns:
                    ...
                    # Se forem planilhas de escalas antigas (busca textual)
    ...
    # TABS DE CONFERÊNCIA
    st.subheader("📋 Conferência de Regras e Equidade Contínua")
    t1, t2, t3 = st.tabs(["👥 Duplas Impedidas", "🚫 Restrições de Função", "🍷 Ranking Acumulado"])
    ...
    with t3: 
        st.write("Abaixo está o ranking considerando todos os históricos enviados:")
    ...
    # RESTAURADO: FILTRO DE EXCLUSÃO DE DATAS
    data_inicio_mes = date(ano, mes, 1)
    if mes == 12: data_fim_mes = date(ano + 1, 1, 1) - timedelta(days=1)
    else: data_fim_mes = date(ano, mes + 1, 1) - timedelta(days=1)
    datas_excluir = st.sidebar.multiselect("Datas para EXCLUIR (Não haverá escala)", options=pd.date_range(data_inicio_mes, data_fim_mes), format_func=lambda x: x.strftime('%d/%m/%Y'))
    ...
    if st.sidebar.button("Gerar Escala Atualizada"):
        ...
        for data in datas_mes:
            ...
            # APLICAÇÃO DA REGRA DE EXCLUSÃO
            ...
            if nome_col_dia in dias_semana:
                ...
                for vaga in vagas:
                    ...
                    # EQUIDADE CONTÍNUA: Prioriza histórico acumulado se for Ceia
                    if data_atual == data_ceia:
                        v_cands = v_cands.sort_values(by=['historico_ceia', 'escalas_no_mes'])
                    else:
                        v_cands = v_cands.sort_values(by='escalas_no_mes')
        ...
        # Consolidação do novo Histórico (Soma o antigo + o que aconteceu agora)
    ...
    if st.session_state.escala_gerada is not None:
        ...
        with c1:
            ...
            st.download_button("📥 Baixar Escala (Excel)", out_esc.getvalue(), f"escala_{mes}_{ano}.xlsx", key="down_esc_v5")
        with c2:
            out_h = io.BytesIO()
            st.session_state.df_memoria.to_csv(out_h, index=False)
            st.download_button("💾 Baixar Histórico ACUMULADO", out_h.getvalue(), "historico_consolidado.csv", key="down_hist_v5")
else:
