...
st.set_page_config(page_title="Gerador de Escala Diaconato V5.9.4", layout="wide")
...
st.title("⛪ Gerador de Escala de Diaconato (Versão 5.9.4)")
...
if arquivo_carregado:
    ...
    t1, t2, t3 = st.tabs(["👥 Duplas Impedidas", "🚫 Restrições", "🍷 Ranking Ceia"])
        ...
        for dia_idx, data in enumerate(datas_mes):
            ...
            if nome_col in dias_semana:
                ...
                if data_atual == data_ceia:
                    # Santa Ceia com todos os escalados do dia para garantir visualização
                    dia_escala["Santa Ceia"] = " | ".join(escalados_dia)
    ...
    if st.session_state.escala_gerada is not None:
        ...
        with c1:
            ...
            st.download_button("📥 Excel Formatado", output.getvalue(), f"Escala_{nome_mes_sel}_{ano_sel}.xlsx")
            
        with c2:
            # --- LÓGICA DE GERAÇÃO DE IMAGEM MELHORADA (V5.9.4) ---
            df_img = st.session_state.escala_gerada.fillna("---").astype(str)
            
            # Função para quebrar texto longo
            def wrap_text(text, width=20):
                if "|" in text: # Especial para Santa Ceia
                    text = text.replace(" | ", "\n")
                return "\n".join(textwrap.wrap(text, width)) if len(text) > width else text

            # Aplicar quebra de texto em todas as colunas
            for col in df_img.columns:
                df_img[col] = df_img[col].apply(lambda x: wrap_text(x, width=25))

            # Calcular altura dinâmica: mais linhas no texto = mais altura na célula
            max_lines = df_img.apply(lambda row: row.str.count('\n').max(), axis=1).sum()
            fig_height = (len(df_img) * 0.6) + (max_lines * 0.2) + 2
            ...
            for (i, j), cell in tab.get_celld().items():
                ...
                if i == 0: # Cabeçalho
                    ...
                    # Se for a coluna da Santa Ceia (geralmente a última ou penúltima)
                    # Vamos alinhar à esquerda se houver muitas quebras de linha
                    if "\n" in cell.get_text().get_text():
                        cell.set_text_props(ha='left')
                
                # Ajuste de altura da célula baseado no conteúdo
                cell.set_height(0.05 + (df_img.iloc[i-1].iloc[j].count('\n') * 0.025) if i > 0 else 0.04)

            buf = io.BytesIO()
            plt.savefig(buf, format='png', bbox_inches='tight', dpi=300, facecolor='white')
            st.download_button("📸 Imagem WhatsApp (Ajustada)", buf.getvalue(), f"Escala_{nome_mes_sel}_{ano_sel}.png")
